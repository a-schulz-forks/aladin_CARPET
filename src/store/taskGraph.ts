...
const mutations = {
  RESET(state: IState) {
    ...
    // Gamification
    // Reset computed paths if currentTask is changed
    if (path === "currentTask") {
      useGamifyStore().computedPaths.value = [];
    }
...
const actions = {
  trackMouse: async ({ commit }, payload) => {
    ...
    gamifyStore.setCheckPath(payload.path);
  },
};
const getters = {
  getPropertyFromPath: (state: IState) => (path: string) => {
    ...
    return splitPath.reduce((value, key) => {
      if (value && Object.keys(value).includes(key)) return value[key]; else return null;
    }, state);
  },
};
